...
`cobalt-plat-terraform-forge` is centered on platform engineering work in Java. The technical target is: Package a Java local lab for terraform analysis with log and snapshot fixtures, replay consistency checks, and documented operating limits.
...
The score is deterministic. The current threshold is 180. Risk has a penalty of 7, latency has a penalty of 4, and weight contributes a bonus of 2.
